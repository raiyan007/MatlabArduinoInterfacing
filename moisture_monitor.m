...
% Last Modified by GUIDE v2.5 08-Dec-2016 04:28:04
...
global a;
a=arduino('COM3');
a.configureDigitalPin(7,'output');
a.configureDigitalPin(13,'output');
a.configureAnalogPin(0,'input');
...
%handles.thresh=str2double(handles.data1);
%plot(0,handles.thresh,'LineWidth',1,'color','red');
%guidata(hObject, handles);
...
%sample=str2double(handles.data2);
%guidata(hObject, handles);
...
% --- Executes on button press in strt.
...
% --- Executes on button press in pltt.
function pltt_Callback(hObject, eventdata, handles)
% hObject    handle to pltt (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global a;
n=0;
thresh=get(handles.edit1,'string');
sample=get(handles.edit2,'string');
datasample=str2double(sample);
m=str2double(thresh);
s=2*m;
for k=1:datasample
    b=a.readVoltage(0);
